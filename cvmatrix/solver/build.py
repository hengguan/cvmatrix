...
# from cvmatrix.utils.config import CfgNode
...
def build_optimizer(cfg, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    """
    import omegaconf
    if cfg.solver.optimizer.type == "AdamW":
        opt_cfg = cfg.solver.optimizer.copy()
        _name = opt_cfg.pop("type")
        if isinstance(opt_cfg, omegaconf.dictconfig.DictConfig):
            opt_cfg = omegaconf.OmegaConf.to_object(opt_cfg)
        optimizer_args = opt_cfg
...
def get_simple_optimizer_params(model: torch.nn.Module):
...
def build_lr_scheduler(
    ...
    """
    lr_scheduler_args = cfg.solver.lr_scheduler
    name = lr_scheduler_args.pop('type')
    ...
    if name == "WarmupMultiStepLR":
        ...
        scheduler_args = dict(cfg.solver.lr_scheduler)
        return torch.optim.lr_scheduler.OneCycleLR(
            optimizer, **scheduler_args)
    elif name == "CyclicLR":
        scheduler_args = dict(lr_scheduler_args)
