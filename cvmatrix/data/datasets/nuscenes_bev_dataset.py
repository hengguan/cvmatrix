# -*- encoding: utf-8 -*-
'''
@File    :   nuscenes_bev_dataset.py
@Time    :   2022-11-02 13:57:24
@Author  :   Guan Heng 
@Version :   v0.1
...
from transforms.nuscenes_cvt import (
    INTERPOLATION, 
    get_view_matrix, 
    get_pose, 
    # get_split,
...
def get_data(
    ...
    **dataset_kwargs
    ...
    transform = SaveDataTransform(Path(labels_dir)/'cvt_labels_nuscenes_v2')

    # Format the split name
    split = f'mini_{split}' if version == 'v1.0-mini' else split
    # split_scenes = get_split(split, 'nuscenes')
    split_path = Path(labels_dir) / f'{split}.txt'
    ...
    for scene_name, scene_record in helper.get_scenes():
        ...
        result.append(data)
    print(f'number of not use: {count_unuse}, split scenes: {len(split_scenes)}')
    return result
...
class NuScenesBEVDetDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
            ...
            transform=None,
            cameras=[[0, 1, 2, 3, 4, 5]],
            world_range={'h_meters': 100, 'w_meters': 100},):
        ...
        if version == 'v1.0-trainval':
            scenes = splits.train if split=='train' else splits.val
        elif version == 'v1.0-test':
            scenes = splits.test
        elif version == 'v1.0-mini':
            scenes = splits.mini_train if split=='train' else splits.mini_val
        else:
        ...
        scenes_tokens = [all_scenes[all_scenes_name.index(s)]['token'] for s in scenes]
        print(f'length of scenes: {len(scenes_tokens)}')

        self.samples = []
        # for st in scenes_tokens:
        #     scene_rec = self.nusc.get('scene', st)
        #     for k, v in scene_rec.items():
        #         print(k)
...
if __name__ == "__main__":
    ...
    dataset = NuScenesBEVDetDataset(dataset_dir, version, split)
    exit()
    print('concat datasets finished!!!')
    print(f'length of datasets: {len(dataset)}')
    loader_cfg = dict(
        batch_size=1,
        num_workers=4,
        pin_memory=True,
    ...
    shuffle = False
    data_loader = torch.utils.data.DataLoader(dataset, shuffle=shuffle, **loader_cfg)
    print('build dataloader success!!!')
    data_loader_iter = iter(data_loader)
    for _ in range(10):
        batch = next(data_loader_iter)
        print('print a batch keys ....')
