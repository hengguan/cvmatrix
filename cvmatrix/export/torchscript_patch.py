...
import cvmatrix  # noqa F401
from cvmatrix.structures import Boxes, Instances
from cvmatrix.utils.env import _import_file
...
def patch_instances(fields):
    """
    A contextmanager, under which the Instances class in cvmatrix is replaced
    by a statically-typed scriptable class, defined by `fields`.
    See more in `scripting_with_instances`.
    """

    with tempfile.TemporaryDirectory(prefix="cvmatrix") as dir, tempfile.NamedTemporaryFile(
...
def patch_builtin_len(modules=()):
    """
    Patch the builtin len() function of a few cvmatrix modules
    to use __len__ instead, because __len__ does not convert values to
    integers and therefore is friendly to tracing.

    Args:
        modules (list[stsr]): names of extra modules to patch len(), in
            addition to those in cvmatrix.
    """
...
def patch_nonscriptable_classes():
    """
    Apply patches on a few nonscriptable cvmatrix classes.
    ...
    from cvmatrix.model.backbone import ResNet, FPN
    ...
    from cvmatrix.model.roi_heads import StandardROIHeads
