...
class NuscDetEval(DatasetEvaluator):
    ...
    def __init__(self,
         eval_modes=['boxes_3d'],
         jsonfile_prefix='./outputs/test',
         eval_version='detection_cvpr_2019',
         modality=None,
         nusc_config=dict(version='v1.0-trainval', dataroot=None, verbose=False)):
        super().__init__()
        self.eval_modes = eval_modes
        self.nusc_annos = dict()
        self.eval_version = eval_version
        self.nusc_config = nusc_config
        self.jsonfile_prefix = jsonfile_prefix

        self.eval_detection_configs = config_factory(self.eval_version)
        from omegaconf import OmegaConf
        self.modality = OmegaConf.to_object(modality)
    ...
    def process(self, inputs, outputs):
        ...
        for sample_id, det in enumerate(outputs):
            annos = []
            boxes = output_to_nusc_box(det)
            sample_token = inputs['metas'][sample_id]["token"]
            info = dict(
                lidar2ego=inputs['lidar2ego'][sample_id].cpu().numpy(),
                ego2global=inputs['ego2global'][sample_id].cpu().numpy(),
            )
    ...
    def evaluate(self):
        ...
        if "masks_bev" in self.eval_modes:
            metrics.update(self.evaluate_map(results))

        if "boxes_3d" in self.eval_modes:
            nusc_submissions = {
                "meta": self.modality,
                "results": self.nusc_annos,
            }
            
            mmcv.mkdir_or_exist(self.jsonfile_prefix)
            res_path = osp.join(self.jsonfile_prefix, "results_nusc.json")
            
            mmcv.dump(nusc_submissions, res_path)

            metrics.update(self._evaluate_single(res_path))
    ...
    def _evaluate_single(
        ...
        print('Build nuscenes ...')
        nusc_ = NuScenes(**self.nusc_config)
        eval_set_map = {
            "v1.0-mini": "mini_val",
            "v1.0-trainval": "val",
        }
        ...
        nusc_eval = DetectionEval(
            ...
            eval_set=eval_set_map[self.nusc_config.version],
...
def lidar_nusc_box_to_global(
    ...
    for box in boxes:
        ...
        box.rotate(pyquaternion.Quaternion(matrix=info["lidar2ego"][:3, :3]))
        box.translate(np.array(info["lidar2ego"][:3, 3]))
        ...
        box.rotate(pyquaternion.Quaternion(matrix=info["ego2global"][:3, :3], atol=1e-7))
        box.translate(np.array(info["ego2global"][:3, 3]))
        box_list.append(box)
    return box_list