...
from cvmatrix.export import TracingAdapter
...
class FlopCountAnalysis(fvcore.nn.FlopCountAnalysis):
    """
    Same as :class:`fvcore.nn.FlopCountAnalysis`, but supports cvmatrix models.
    """
...
def flop_count_operators(model: nn.Module, inputs: list) -> typing.DefaultDict[str, float]:
    ...
    Args:
        model: a cvmatrix model that takes `list[dict]` as input.
        inputs (list[dict]): inputs to model, in cvmatrix's standard format.
