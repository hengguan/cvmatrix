...
from cvmatrix.model.layers import ShapeSpec
from cvmatrix.structures import BitMasks, Boxes, ImageList, Instances
...
class MMDetBackbone(Backbone):
    """
    Wrapper of mmdetection backbones to use in cvmatrix.

    mmdet backbones produce list/tuple of tensors, while cvmatrix backbones
    produce a dict of tensors. This class wraps the given backbone to produce
    output in cvmatrix's convention, so it can be used in place of cvmatrix
    backbones.
    """
...
class MMDetDetector(nn.Module):
    ...
    Input/output formats of this class follow cvmatrix's convention, so a
    mmdetection model can be trained and evaluated in cvmatrix.
    """
