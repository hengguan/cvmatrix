...
class CrossViewAttention(nn.Module):
    ...
    def forward(
        ...
        #compute angle range
        # rads = torch.atan2(rays[:, 1, :1, :], rays[:, 0, :1, :])                # (b n) 1 w
        # rads_max = rearrange(rads[..., :1], '(b n) c d -> b n c d', b=b, n=n)           # b n 1 1
        # rads_min = rearrange(rads[..., -1:], '(b n) c d -> b n c d', b=b, n=n)           # b n 1 1
        ...
        # compute angle
        # bev_rads = torch.atan2(bevs[:, 1, :, :], bevs[:, 0, :, :]) # (b n) H W
        # bev_rads = rearrange(bev_rads, '(b n) H W -> b n H W', b=b, n=n)
        # bevmin = bev_rads >= rads_min
        # bevmax = bev_rads <= rads_max
        # mask = bevmax.clone()
        # mask[:, [0, 1, 2, 3, 5], :, :] = bevmax[:, [0, 1, 2, 3, 5], :, :] * bevmin[:, [0, 1, 2, 3, 5], :, :]
        # mask[:, 4, :, :] = bevmax[:, 4, :, :] + bevmin[:, 4, :, :]
        # mask = rearrange(mask, 'b n H W -> (b n) H W')

        # bevs = F.pad(bevs, (0, 0, 0, 0, 0, 1, 0, 0), value=0)    # (b n) 4 H W
        # bevs = bevs + c_flat[:, :2, :, :]
        # world_ = self.world_embed(world[None])                     # 1 d/2 H W
        # world_ = repeat(world_, 'b ... -> (repeat b) ...', repeat=b)                     # bn 2 H W
        # bevs = torch.cat([bevs, world_], axis=1)
        # bevs *= mask[:, None]
        # bevs = rearrange(bevs, '(b n) d H W -> b n d H W', b=b, n=n)
        # bevs = bevs.sum(dim=1)                                                 # b d H W
