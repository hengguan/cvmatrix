...
__all__ = ['SimpleCrossViewTransformer']
...
class CrossViewAttention(nn.Module):
    def __init__(
        ...
        # in_dim = 6 * feat_height * feat_width
        # out_dim = 25 * 25
        # ffn_dim = 128
    ...
    def forward(
        ...
        return query_pos, key, val
        

@TRANSFORMER_REGISTRY.register()
class CVTSimple(nn.Module):
    def __init__(
            self,
            *,
            # backbone,
        ...
        heads, dim_head, qkv_bias, skip, _, _, _ = tuple(cross_view.values())
        cross_views = list()
        # layers = list()
        cross_atts = list()
        ...
        for feat_shape, num_layers in zip(feats_shapes, middle):
            ...
            ca = CrossAttention(dim, heads, dim_head, qkv_bias)
            cross_atts.append(ca)

        self.bev_embedding = BEVEmbedding(dim, **bev_embedding)
        self.cross_views = nn.ModuleList(cross_views)
        # self.layers = nn.ModuleList(layers)
        self.layers = nn.Sequential(*[ResNetBottleNeck(dim) for _ in range(middle[0])])

        self.cross_attends = nn.ModuleList(cross_atts)
        self.dim = dim
        self.skip = skip
    ...
    def forward(self, feats, camera):
        ...
        for cross_view, feature in zip(self.cross_views, features): # , layer , self.layers
            ...
            q_pos, k, v = cross_view(x, self.bev_embedding, feature, I_inv, E_inv)
            for cross_attend in self.cross_attends:
                q = q_pos + x[:, None]
                x = cross_attend(q, k, v, skip=x if self.skip else None)
                # q = x.clone()
            x = self.layers(x)
            # down = False
