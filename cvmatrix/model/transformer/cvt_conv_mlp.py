...
class CrossViewAttention(nn.Module):
    def __init__(
        ...
        cross_feat_dim = (dim + 4) * 6
        self.feature_linear = nn.Sequential(
            nn.BatchNorm2d(cross_feat_dim),
            nn.ReLU(),
            nn.Conv2d(cross_feat_dim, dim, 1, bias=False),
        )
        ...
        # MLP for converting feature with geometric and apperance info to bev feature query
        in_dim = feat_height * feat_width
        out_dim = 25 * 25
        ffn_dim = 128
    ...
    def forward(
        ...
        img_embed = self.img_embed(d_flat)                                        # (b n) d h w
        ...
        feat_depth = torch.cat((d_flat, feat_proj), dim=1)
        feat_depth = rearrange(feat_depth, '(b n) c h w -> b (n c) h w', b=b, n=n)
        feat_depth = self.feature_linear(feat_depth)                                # (b n) d h w 
        feat_depth = rearrange(feat_depth, 'b c h w -> b c (h w)', b=b)
        feat_bev = self.MLP(feat_depth)

        _, H, W = world.shape
        feat_bev = rearrange(feat_bev, 'b c (h w) -> b c h w', h=H, w=W)       # b n c (h w)
        if x is not None:
            feat_bev += x 
            
        # Expand + refine the BEV embedding
        query = query_pos.squeeze(1) + feat_bev # x[:, None]                    # b n d H W
        key = rearrange(key_flat, '(b n) ... -> b n ...', b=b, n=n)             # b n d h w
        val = rearrange(feat_proj, '(b n) ... -> b n ...', b=b, n=n)              # b n d h w

        return self.cross_attend(query, key, val, skip=feat_bev if self.skip else None)


@TRANSFORMER_REGISTRY.register()
class CVFormer(nn.Module):
    def __init__(
            self,
            *,
            # backbone,
        ...
        for feat_shape, num_layers, s in zip(feats_shapes, middle, scales):
            _, _, h, w = feat_shape
